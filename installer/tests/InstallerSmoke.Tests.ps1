...
$setupPath = Join-Path $PSScriptRoot '..\dist\LiveAvatar-Setup-0.1.2.exe'
...
if (Test-Path -LiteralPath $setupPath -PathType Leaf) {
    ...
    Assert-True ([string]$setup.VersionInfo.ProductVersion -match '^0\.1\.2') 'setup embeds version 0.1.2'
}
