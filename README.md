...
- 🖼️ **图片头像管理** — 在“设置”中上传单人正脸图片、裁剪、生成并无缝切换数字人头像。
...
运行 `LiveAvatar-Setup-0.1.4.exe`，选择程序目录和数据目录即可。安装器本身约 2.1 MB，不内置模型；所有载荷支持断点续传、固定字节数和 SHA-256 校验。RTX 50/Blackwell 使用 PyTorch 2.11.0 CUDA 12.8。0.1.4 会让竖屏数字人保持原比例清晰显示，并用柔化背景补齐横屏空间。安装、迁移、修复、卸载保留规则见 [安装包说明](docs/INSTALLER.md)。
...
要更换静态图片头像，打开右上角“设置”，选择 JPEG、PNG 或 WebP 图片，完成 3:4 裁剪后点击“生成并切换”。图片最大 10 MiB；裁剪结果固定为 576×768。处理过程会显示人脸检查、生成和切换状态，生成成功后原始上传文件不会保留。
